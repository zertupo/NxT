...
next_word<-function(str1){
    ...
    if (length(temp)==1){
        affiche<-head(filter(df2,W1==temp[1]),5)
        affiche<-affiche$W2
        m1<-"ok"
    } else {
        if (length(temp)==2){
            affiche2<-head(filter(df3,W1==temp[1],W2==temp[2]),5)
            affiche2<-affiche2$W3
            if (length(affiche2)==0){
                affiche2<-head(filter(df2,W1==tempo[2]),5)
                affiche2<-affiche2$W2
            }
            m2<-"ok"
        } else{
            tempo<-tail(temp,3)
            affiche3<-head(filter(df4,W1==tempo[1],W2==tempo[2],
                                  W3==tempo[3]),5)
            affiche3<-affiche3$W4
            if (length(affiche3)==0){
                affiche3<-head(filter(df3,W1==tempo[2],W2==tempo[3]),5)
                affiche3<-affiche3$W3
            }
            if (length(affiche3)==0){
                affiche3<-head(filter(df2,W1==tempo[3]),5)
